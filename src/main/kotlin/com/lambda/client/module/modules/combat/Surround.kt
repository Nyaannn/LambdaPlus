...
object Surround : Module(
    ...
    private val placeSpeed by setting("Places Per Tick", 4f, 0.25f..10f, 0.25f)
    ...
    private val SurroundBlucks by setting("Surround Blocks", SurroundBlocks.OBBI)
    ...
    private val SpedLimit by setting("Speed Limit", 0, 0..100, 1)
    private val att by setting("Attempts", 2, 1..25, 1)
    private val toggleMessage by setting("Toggle Message", true);

    private enum class AutoDisableMode {
        ONE_TIME, OUT_OF_HOLE
    }
    private enum class SurroundBlocks {
        OBBI,ECHEST,BEDROCK,COBWEB
    }
    ...
    fun SafeClientEvent.inHoleCheck() = player.onGround && player.speed < SpedLimit && checkHole(player) == SurroundUtils.HoleType.OBSIDIAN
    ...
    private fun SafeClientEvent.getObby(): HotbarSlot? {
            ...
            var slots = player.hotbarSlots.firstBlock(Blocks.OBSIDIAN)
            if(SurroundBlucks==SurroundBlocks.OBBI){slots = player.hotbarSlots.firstBlock(Blocks.OBSIDIAN)}
            if(SurroundBlucks==SurroundBlocks.ECHEST){slots = player.hotbarSlots.firstBlock(Blocks.ENDER_CHEST)}
            if(SurroundBlucks==SurroundBlocks.BEDROCK){slots = player.hotbarSlots.firstBlock(Blocks.BEDROCK)}
        if(SurroundBlucks==SurroundBlocks.COBWEB){slots = player.hotbarSlots.firstBlock(Blocks.WEB)}

        if (slots == null) { // Obsidian check
                if (isEnabled) {
                    if (toggleMessage) MessageSendHelper.sendChatMessage("$chatName No usable blocks in hotbar, disabling!")
                    disable()
                }
                return null
            }
