...
object MessageSendHelper {
    ...
    fun sendChatMessage(message: String) {
        if(NotifColor.chatcolorslider == NotifColor.clor.PINK){
        sendRawChatMessage(coloredName('d') + message)
        }
        if(NotifColor.chatcolorslider == NotifColor.clor.BLACK){
            sendRawChatMessage(coloredName('8') + message)
        }
        if(NotifColor.chatcolorslider == NotifColor.clor.RED){
            sendRawChatMessage(coloredName('c') + message)
        }
        if(NotifColor.chatcolorslider == NotifColor.clor.GREEN){
            sendRawChatMessage(coloredName('2') + message)
        }
        if(NotifColor.chatcolorslider == NotifColor.clor.BLUE){
            sendRawChatMessage(coloredName('9') + message)
        }
        if(NotifColor.chatcolorslider == NotifColor.clor.PURPLE){
            sendRawChatMessage(coloredName('5') + message)
        }
    }
    ...
    private fun coloredName(colorCode: Char) = "&7[&${NotifColor.colorcode}" + LambdaMod.LAMBDA + "&7] &r"
